    ...
    <BannerLayout>
      <div className='grid md:grid-cols-2 md:divide-x-4 md:divide-Green px-4 pb-2 pt-10'>
        <div className='order-2 md:order-1  md:mr-12'>
          ...
          {isLoading
            ? [1, 2, 3].map((item) => <ParagraphSkeleton key={item} className={'p-8 w-full relative'} />)
            : data && data[0]?.eduCards?.map((data, key) => <Edu_Card key={key} data={data} />)}
        </div>
        <div className='order-1 md:order-2'>
          <div className='md:ml-12'>
            ...
            {isLoading
              ? [1, 2, 3, 4].map((item) => <ParagraphSkeleton key={item} className={'p-8 h-full w-full relative'} />)
              : data && data[1]?.expCards?.map((data, key) => <Exp_Card key={key} data={data} />)}
          </div>
        </div>
      </div>
      <Footer />
